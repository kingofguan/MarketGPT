...
class ITCH_Dataset(Dataset):
    ...
    def get_masking_fn(*, random_msg_idxs=None, random_fields=None, randomize_message=True):
        ...
        if random_fields is not None:
            ...
            random_msg_idxs = [
                idx 
                for f in field_idxs
                for idx in range(*ITCH_Dataset._get_tok_slice_i(f))]
    ...
    def random_mask(seq, rng, exclude_time=True):
        ...
        if exclude_time:
            ...
            time_start_i, time_end_i = ITCH_Dataset._get_tok_slice_i(time_field_i)
        ...
        if exclude_time:
            ...
            dt_idx = list(range(*ITCH_Dataset._get_tok_slice_i(dt_field_i)))
    ...
    def causal_mask(seq, rng):
        ...
        """
        seq = seq.copy()
        hidden_fields, msk_field = ITCH_Dataset._select_sequential_causal_mask_no_time(rng)

        i_start, i_end = ITCH_Dataset._get_tok_slice_i(msk_field)
        ...
        for f in hidden_fields:
            seq[-1][slice(*ITCH_Dataset._get_tok_slice_i(f))] = Vocab.HIDDEN_TOK
            # seq = seq.at[-1, slice(*ITCH_Dataset._get_tok_slice_i(f))].set(Vocab.HIDDEN_TOK)
        return seq, y
...
class ITCH_Subset(Subset):
    def __init__(self, dataset: ITCH_Dataset, indices: Sequence[int]) -> None:
...
class ITCH(SequenceDataset):
    ...
    def init_defaults(self):
        ...
        return {
            ...
            "mask_fn": ITCH_Dataset.random_mask,
            "use_book_data": False,
            "use_simple_book" : False,
    ...
    def setup(self):
        ...
        print("n_test_files=", n_test_files)
        n_test_files = 0
        ...
        #n_cache_files = 0
        self.dataset_train = ITCH_Dataset(
            self.train_files,
        ...
        # self.dataset_test = ITCH_Dataset(
        #     self.test_files,
        #     n_messages=self.n_messages,
        #     mask_fn=self.mask_fn,
        #     seed=self.rng.randint(0, sys.maxsize),
